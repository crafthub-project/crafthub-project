...
const learningSkills = [
  { name: 'Bread Baking',     emoji: '🍞', progress: 75, categoryKey: 'baking',   nextLesson: 'Advanced Techniques' },
  { name: 'Basket Weaving',   emoji: '🧺', progress: 45, categoryKey: 'weaving',  nextLesson: 'Traditional Patterns' },
  { name: 'Soap Making',      emoji: '🧼', progress: 30, categoryKey: 'soap',     nextLesson: 'Natural Ingredients'  },
  { name: 'Machine Tailoring',emoji: '🧵', progress: 60, categoryKey: 'tailoring',nextLesson: 'Dress Patterns'       },
];
...
export default function Dashboard() {
  ...
  const kpiCards = [
    {
      label: t('activeSkillsLabel'),
      value: '8',
      icon: BookOpen,
      trend: t('trendActiveSkills'),
      trendUp: true,
      iconBg: '#EEF2FF',
      iconColor: '#4F46E5',
    },
  ...
  const recentActivity = [
    { icon: '🎉', text: t('trendOrders') !== '+3 new orders' ? `${t('lessonsDone')}: Bread Baking Basics` : 'Completed lesson "Bread Baking Basics"', time: '2h ago', color: '#ECFDF5' },
    { icon: '🛒', text: 'New order: Vanilla Cake — 15,000 UGX', time: '5h ago', color: '#FEF9E7' },
    { icon: '⭐', text: 'Received 5-star review from Mary K.',  time: t('previous'),  color: '#FFF7ED' },
    { icon: '📦', text: 'Product listed: Handwoven Basket',      time: '2d ago',       color: '#EEF2FF' },
  ];
  ...
  return (
    <div className="space-y-6 max-w-screen-2xl">
      ...
      >
        ...
        <div className="absolute -bottom-12 right-28 w-36 h-36 rounded-full opacity-5" style={{ backgroundColor: GOLD }} />
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`flex items-center gap-0.5 text-xs font-semibold px-2 py-1 rounded-full ${
                    card.trendUp ? 'text-emerald-700 bg-emerald-50' : 'text-red-600 bg-red-50'
                  }`}
                >
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <h2 className="font-semibold text-slate-900">{t('learningProgress')}</h2>
                <p className="text-xs text-slate-500 mt-0.5">
                  {learningSkills.length} {t('activeSkillsProgress')}
                </p>
              </div>
              <button
                    ...
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: skill.progress >= 70 ? GREEN : skill.progress >= 40 ? GOLD : NAVY,
                        }}
                      />
                    </div>
                    <p className="text-xs text-slate-400 mt-1">
                      {t('nextLessonLabel')} {skill.nextLesson}
                    </p>
                  </div>
                  <button
                    onClick={() => navigate('/learn')}
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{t('yourLatestUpdates')}</p>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-500 mt-0.5">{t('shortcutsKeyTasks')}</p>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-sm">{t('communityStats')}</h2>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="font-semibold text-slate-900 text-sm">{t('performanceTitle')}</h2>
                <p className="text-xs text-slate-500 mt-0.5">{t('thisMonth')}</p>
              </div>
